...
sampling_rate = 0.10
sample_size = int(len(true_accuracies) * sampling_rate)
np.random.seed(42) 
sampled_indices = np.random.choice(len(true_accuracies), size=sample_size, replace=False)
sampled_accuracies = true_accuracies[sampled_indices]
print(f"Number of Ground Truth Subsets: {len(true_accuracies)}")
...
np.random.seed(42)
uniform_dist = np.random.uniform(0, 1, size=len(true_accuracies))
...
np.random.seed(42)
gaussian_dist = np.random.normal(0.5, 0.1, size=len(true_accuracies))
...
plt.figure(figsize=(18, 12))
n, bins, patches = plt.hist(accuracies_rf, bins=50, edgecolor='black', color='sandybrown', alpha=0.7)
...
# Combined NN vs RF distribution with summary table
